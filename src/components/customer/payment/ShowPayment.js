  ...
  render() {
    ...
    if (this.state.active === "pay") {
      content = (
        <div class="flex-box">
          <br />
          <p>Fyll i kortuppgifter för at betala månadens resor.</p>
          <br />
      ...
      content = <p>Du har inga väntande betalningar.</p>;
    } else {
      content = (
          ...
          {this.state.months.map((trip) => {
            ...
            return (
              <div className="trips-div">
                ...
                <p>Summa: {trip[1]}</p>
                <br />
                {this.state.user.payment_method[0] === "monthly" && trip[2] ? (
                  <p>Status: {trip[2]}</p>
                ) : (
                  <p>Betald med saldo</p>
                )}
                <br />
                {trip[3] ? (
                  <p>Dagar kvar till betalning: {trip[3]}</p>
                ) : (
                  <p>
                    Du måste betala för att kunna fortsätta resa denna månad
                  </p>
                )}
                <br />
                {trip[2] === "unpaid" ? (
                  <button
                    onClick={() => {
                      this.setState({ active: "pay" });
                    }}
                  >
                    Betala
                  </button>
                ) : (
                  <p>Denna månad är betalad</p>
                )}
              </div>
            );
          })}
    ...
    if (this.state.render) {
      renderContainer = <div>{content}</div>;
    }
